...
const GuideStatistique = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        console.log("STATS GUIDES => ", data);

    const mappedStats = [
        { icon: Users, value: data.total, label: "Total guides", delta: "+0", positive: true },
        { icon: Users, value: data.actif, label: "Guides actifs", delta: "+0", positive: true },
        { icon: Users, value: data.inactif, label: "Guides inactifs", delta: "+0", positive: false },
        ];

        setStats(mappedStats);
      } catch (error) {
        console.error("Erreur chargement stats guides :", error);
      }
    };
  ...
  if (!stats.length)
    return <p className="p-6">Chargement des statistiques guides...</p>;
...
export default GuideStatistique;