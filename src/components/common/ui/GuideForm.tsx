import { AlertCircle, CheckCircle, Compass, Eye, EyeOff, Globe, Lock, MailCheck, MapPinHouseIcon, PhoneCall, User, X } from "lucide-react";
import { motion } from "framer-motion";
import { InputText, Label } from "./Input";
import { useState, useEffect } from "react";
import { inscrireGuide, updateGuide } from "../../../service/AuthService";

const GuideForm = ({ open, onClose, onSuccess, initialData = null }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setForm({
        ...
        preferenceTouristique: initialData.preferenceTouristique ?? "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await updateGuide(initialData.id, {
          ...
          preferenceTouristique: form.preferenceTouristique,
      ...
      //setSuccess sorti du else — fonctionne pour create ET edit
      setSuccess(isEdit ? "Guide modifié avec succès !" : "Guide créé avec succès !");
      setTimeout(() => {
        setSuccess(null);
        onSuccess?.();
        onClose();
      ...
      const message = err.response?.data?.message
        || `Erreur lors de la ${isEdit ? "modification" : "création"} du guide.`;
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      ...
      >
        <div className="flex flex-col max-h-[90vh] bg-white rounded-2xl">

          {/* Header */}
          <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100 flex-shrink-0">
            <h2 className="text-xl font-bold">
              {isEdit ? "Modifier le guide" : "Nouveau guide"}
            </h2>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 font-medium mb-2">
                    <Compass size={18} />
                    <Label label="Préférence" />
                  </div>
                  <InputText type="text" name="preferenceTouristique" placeholder="Culturel, Historique"
                    value={form.preferenceTouristique} onChange={handleChange} />
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-2 font-medium mb-2">
                    <Compass size={18} />
                    <Label label="Expérience (ans)" />
                  </div>
                  <InputText type="number" name="experience" placeholder="4"
