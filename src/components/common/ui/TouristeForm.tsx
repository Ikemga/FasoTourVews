import {
    Compass, Eye, EyeOff, Globe, Lock,
    MailCheck, MapPinHouseIcon, PhoneCall, User, X,
} from "lucide-react";
import { AlertCircle, CheckCircle } from "lucide-react";
import { motion } from "framer-motion";
import { InputText, Label } from "./Input";
import { useState, useEffect } from "react";
import { inscrireTouriste, updateTouriste } from "../../../service/AuthService";

const TouristeForm = ({ open, onClose, onSuccess, initialData = null }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                await updateTouriste(initialData.id, {
            ...
            setSuccess(isEdit ? "Touriste modifié avec succès !" : "Touriste créé avec succès !");
            setTimeout(() => {
                setSuccess(null);
                onSuccess?.();
                onClose();
            ...
            const message = err.response?.data?.message
                || `Erreur lors de la ${isEdit ? "modification" : "création"} du touriste.`;
            setError(message);
        } finally {
            setLoading(false);
        }
                ...
                <div className="bg-white w-full rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
                    ...
                    <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100 flex-shrink-0">
                        <h2 className="text-xl font-bold">
                            {isEdit ? "Modifier le touriste" : "Nouveau touriste"}
                        </h2>
