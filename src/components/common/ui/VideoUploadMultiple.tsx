...
const VideoUploadMultiple = ({ onChange }) => {
    const [videos, setVideos] = useState([]);

    const handleVideos = (e) => {
        const files = Array.from(e.target.files);
        const newVideos = files.map(file => ({ file, preview: URL.createObjectURL(file) }));
                ...
                <div className="flex flex-wrap gap-2">
                    {videos.map((vid, index) => (
                        <div key={index} className="relative w-16 h-16 flex-shrink-0">
                            <video src={vid.preview} className="w-full h-full object-cover rounded-xl bg-black" muted preload="metadata" />
