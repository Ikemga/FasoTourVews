...
const normalize = (items) =>
  items.map((item) =>
    typeof item === "string"
      ? { file: null, preview: getImageUrl(item) ?? item }
...
const ImageUploadMultiple = ({ existantes = [], onChange }) => {
  ...
  // Se relance chaque fois qu'existantes change
  useEffect(() => {
    if (existantes.length > 0) {
      const normalized = normalize(existantes);
      setImages(normalized);
      // Remonter les valeurs initiales au parent
      onChange?.(normalized.map((i) => i.file ?? i.preview));
    }
  }, [existantes]);  // ← dépendance clé
